# Learning This Repo with /master

Everything else in this repo assumes you're working through notebooks and
reference pages by hand. `/master` is the interactive alternative: one
...
|---|---|---|
| `/master` | **Lesson** — teaches the next concept you haven't mastered yet, one question at a time, adapting depth to your answers | Default. Use this most of the time — it's the one that actually builds understanding |
| `/master quiz` | 10 rapid multiple-choice questions across different concepts, right/wrong revealed immediately | A fast gut-check across many topics, or a warm-up before a lesson |
| `/master quiz 20` | Same, a bigger round | When you have more time and want broader coverage |
| `/master scenario` | Applies concepts you've already confirmed to a real portfolio workflow | Once you've got some concepts confirmed and want to see them used together, not just recited |
| `/master exam` | A tougher, ladder-style oral check across several concepts at once, narrowing in on weak spots | As a checkpoint after finishing a phase (e.g. all of fixed income) |
| `/master status` | Just reports how far along you are — nothing else | Any time you want a quick read without starting a session |
| `/master <topic>` (e.g. `/master duration`) | Jumps straight to a specific concept instead of the default order | When you already know what you want to work on |
...
1. Type `/master`. Say it shows `1/55 · Trainee` with duration already
...
4. Come back anytime — tomorrow, next week — and type `/master` again.
...
- Once every concept is confirmed, `/master` will say so and point you
  at `use_cases/` — applying what you know instead of studying more.

## How this relates to /tutor and /pm-query

`/master` is the recommended default — it's the only command most
...
- **`/tutor <topic>`** — the same kind of teaching session, but for
  ...
  `/master` actually uses this same logic under the hood.
