...
def test_put_call_parity_residual_nonzero_for_inconsistent_quotes():
    ...
    residual = put_call_parity_residual(call_price=11.0, put_price=5.0, spot=S, strike=K, rate=R, time_to_expiry=T)
...
def test_vega_matches_finite_difference():
    eps = 1e-4
    fd = (
        black_scholes_call_price(S, K, R, SIGMA + eps, T) - black_scholes_call_price(S, K, R, SIGMA - eps, T)
    ) / (2 * eps)
...
def test_vega_identical_for_call_and_put():
    ...
    eps = 1e-4
    fd_put = (
        black_scholes_put_price(S, K, R, SIGMA + eps, T) - black_scholes_put_price(S, K, R, SIGMA - eps, T)
    ) / (2 * eps)
...
def test_theta_call_matches_finite_difference():
    eps = 1e-4
    fd = -(
        black_scholes_call_price(S, K, R, SIGMA, T + eps) - black_scholes_call_price(S, K, R, SIGMA, T - eps)
    ) / (2 * eps)
...
def test_theta_put_matches_finite_difference():
    eps = 1e-4
    fd = -(
        black_scholes_put_price(S, K, R, SIGMA, T + eps) - black_scholes_put_price(S, K, R, SIGMA, T - eps)
    ) / (2 * eps)
...
def test_rho_call_matches_finite_difference():
    eps = 1e-4
    fd = (
        black_scholes_call_price(S, K, R + eps, SIGMA, T) - black_scholes_call_price(S, K, R - eps, SIGMA, T)
    ) / (2 * eps)
...
def test_rho_put_matches_finite_difference():
    eps = 1e-4
    fd = (
        black_scholes_put_price(S, K, R + eps, SIGMA, T) - black_scholes_put_price(S, K, R - eps, SIGMA, T)
    ) / (2 * eps)
...
def test_black76_matches_black_scholes_at_the_matching_forward():
    ...
    assert np.isclose(black76_call_price(forward, K, R, SIGMA, T), black_scholes_call_price(S, K, R, SIGMA, T), atol=1e-8)
    assert np.isclose(black76_put_price(forward, K, R, SIGMA, T), black_scholes_put_price(S, K, R, SIGMA, T), atol=1e-8)
