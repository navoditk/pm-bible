...
def test_jensens_alpha_hand_example():
    ...
    # realized=12% -> alpha = 3%
    alpha = jensens_alpha(realized_return=0.12, risk_free_rate=0.03, beta_value=1.2, market_return=0.08)
    assert np.isclose(alpha, 0.03)

def test_jensens_alpha_zero_when_realized_matches_capm():
    alpha = jensens_alpha(realized_return=0.09, risk_free_rate=0.03, beta_value=1.2, market_return=0.08)
