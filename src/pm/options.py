...
def _d1_d2(spot, strike, rate, volatility, time_to_expiry, dividend_yield=0.0):
    d1 = (np.log(spot / strike) + (rate - dividend_yield + 0.5 * volatility**2) * time_to_expiry) / (
...
def put_call_parity_residual(call_price, put_price, spot, strike, rate, time_to_expiry, dividend_yield=0.0):
...
def gamma(spot, strike, rate, volatility, time_to_expiry, dividend_yield=0.0):
    ...
    d1, _ = _d1_d2(spot, strike, rate, volatility, time_to_expiry, dividend_yield)
    return float(np.exp(-dividend_yield * time_to_expiry) * norm.pdf(d1) / (spot * volatility * np.sqrt(time_to_expiry)))
...
def vega(spot, strike, rate, volatility, time_to_expiry, dividend_yield=0.0):
    ...
    """
    d1, _ = _d1_d2(spot, strike, rate, volatility, time_to_expiry, dividend_yield)
    return float(spot * np.exp(-dividend_yield * time_to_expiry) * norm.pdf(d1) * np.sqrt(time_to_expiry))
...
def theta_call(spot, strike, rate, volatility, time_to_expiry, dividend_yield=0.0):
    ...
    """
    d1, d2 = _d1_d2(spot, strike, rate, volatility, time_to_expiry, dividend_yield)
    term1 = -spot * np.exp(-dividend_yield * time_to_expiry) * norm.pdf(d1) * volatility / (2 * np.sqrt(time_to_expiry))
...
def implied_volatility_call(market_price, spot, strike, rate, time_to_expiry, dividend_yield=0.0):
    ...
    """
    def objective(vol):
        return black_scholes_call_price(spot, strike, rate, vol, time_to_expiry, dividend_yield) - market_price
...
def implied_volatility_put(market_price, spot, strike, rate, time_to_expiry, dividend_yield=0.0):
    ...
    def objective(vol):
        return black_scholes_put_price(spot, strike, rate, vol, time_to_expiry, dividend_yield) - market_price
    return float(brentq(objective, 1e-6, 5.0))
...
def black76_put_price(forward, strike, rate, volatility, time_to_expiry):
    ...
    d1 = (np.log(forward / strike) + 0.5 * volatility**2 * time_to_expiry) / (volatility * np.sqrt(time_to_expiry))
    d2 = d1 - volatility * np.sqrt(time_to_expiry)
    return float(np.exp(-rate * time_to_expiry) * (strike * norm.cdf(-d2) - forward * norm.cdf(-d1)))
